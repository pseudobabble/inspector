...
class TestDocuments(unittest.TestCase):
    ...
    def test_get_all(self, mock_flask):
        ...
        documents_resource = Documents(
            document_schema=mock_schema, document_repository=mock_repository
        )
    ...
    def test_post(self, mock_flask):
        mock_flask.request.get_json.return_value = {
            "filename": "test.py",
            "content": "abc123",
        }
        ...
        documents_resource = Documents(
            document_schema=mock_schema, document_builder=mock_builder
        )

        result = documents_resource.post()

        assert result == "DUMP"
        mock_builder_build.assert_called_with(
            RawDocument(filename="test.py", content="abc123")
        )
        mock_schema_dump.assert_called_with(return_document)
    ...
    def test_patch(self, mock_flask):
        ...
        return_document = Document(id=1, filename="test_file.py", raw_content="abc123")
        mock_builder_update_documents_with_ml_documents = Mock(
            return_value=[return_document]
        )
        mock_builder = Mock(
            update_documents_with_ml_documents=mock_builder_update_documents_with_ml_documents
        )

        documents_resource = Documents(document_builder=mock_builder)

        result = documents_resource.patch()

        assert result == {"success": 200}
        mock_builder_update_documents_with_ml_documents.assert_called_with(
            response_json
        )
