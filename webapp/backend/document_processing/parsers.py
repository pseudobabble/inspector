...
class FileParser:
    ...
    def parse(self, file_datum: Dict[str, Any], logger) -> ParsedFile:
        ...
        logger.info("61: BEFORE: /tmp contents: %s\nresult_filename: %s", os.listdir('/tmp'), result_filename)
        parsed_file = self._parse_temp_file(result_filename, original_filename_stem_with_new_extension, logger)

        return parsed_file


    def _parse_temp_file(self, result_filename: str, filename: str, logger):
        logger.info("68: JUST BEFORE: /tmp contents: %s\nresult_filename: %s", os.listdir('/tmp'), result_filename)
...
class WordProcessorXmlParser(FileParser):
    ...
    def process(self, temporary_file_path: Path, logger) -> str:
        ...
        #     shlex.split(f"soffice --headless --convert-to fodt:'OpenDocument Text Flat XML' {temporary_file_path} --outdir {outdir}"),
        #     capture_output=True
        # )
        result = subprocess.run(
            shlex.split("pwd && ls /tmp"),
            shell=True,
            capture_output=True
        )
...
class WordProcessorTextParser(FileParser):
    ...
    def process(self, temporary_file_path: Path, logger) -> str:
        ...
        result = subprocess.run(
            shlex.split(f'soffice --headless --convert-to txt:Text {temporary_file_path} --outdir {outdir}'),
            shell=True
        )
...
#            WordProcessorXmlParser()
    ...
    def parse(self, uploaded_file_data: List[dict], logger) -> List[ParsedFileCollection]:
