...
from document_processing.parsers import WordProcessorXmlParser, WordProcessorTextParser, \
    ParsedFile, ParsedFileCollection, ParserCoordinator
...
class TestFileParsers(unittest.TestCase):
    ...
    def test_WordProcessorXmlParser(self, mock_subprocess):
        ...
        expected_filename = Path('/backend' + '/' + original_filename.split('.')[0] + '.fodt')
    ...
    def test_WordProcessorTextParser(self, mock_subprocess):
        ...
        command = shlex.split(
            f"soffice --headless --convert-to txt:Text {tf.name}"
        )

        result_filename = parser.process(tf)

        expected_filename = Path('/backend' + '/' + original_filename.split('.')[0] + '.txt')
        assert result_filename == expected_filename
        mock_subprocess.run.assert_called_with(command)
...
class TestParserCoordinator(unittest.TestCase):
    ...
    def test_parse(self):
        ...
        parse_to_txt = ParsedFile(filename='test.txt', content=b'some bytes', file_extension='txt')
        mock_parse_to_text_parse = Mock(return_value=parse_to_txt)
        mock_parser_to_text = Mock(parse=mock_parse_to_text_parse, allowed_types=['ext', 'other'])

        parse_to_fodt = ParsedFile(filename='test.fodt', content=b'some bytes', file_extension='fodt')
        mock_parse_to_fodt_parse = Mock(return_value=parse_to_fodt)
        mock_parser_to_fodt = Mock(parse=mock_parse_to_fodt_parse, allowed_types=['ext', 'other'])
        ...
        assert parsed_file_collections == [
            ParsedFileCollection(
                fodt=ParsedFile(filename='test.fodt', content='some bytes', file_extension='fodt'),
                txt=ParsedFile(filename='test.txt', content='some bytes', file_extension='txt'),
                original=ParsedFile(filename='test.ext', content='some bytes', file_extension='ext')
            ),
            ParsedFileCollection(
                fodt=ParsedFile(filename='test.fodt', content='some other bytes', file_extension='fodt'),
                txt=ParsedFile(filename='test.txt', content='some other bytes', file_extension='txt'),
                original=ParsedFile(filename='test.other', content='some other bytes', file_extension='other')
            )
        ]
